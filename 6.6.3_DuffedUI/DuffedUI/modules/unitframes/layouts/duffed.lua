-- Unitframelayout 2 for DuffedUI --
...
if C["unitframes"].layout == 1 then
	...
	if not C["unitframes"].lafo then G.UnitFrames.Focus.Power:Kill() end
	G.UnitFrames.Focus.Power.value:Kill()
	G.UnitFrames.Focus.Health.value:Kill()
	...
	-- player --
	-- panel --
	local Dpanel = CreateFrame("Frame", nil, G.UnitFrames.Player)
	Dpanel:SetTemplate("Default")
	Dpanel:Size(C["unitframes"].playerwidth + 2, 13)
	Dpanel:Point("BOTTOM", G.UnitFrames.Player, "BOTTOM", 0, 0)
	Dpanel:SetFrameLevel(2)
	Dpanel:SetFrameStrata("MEDIUM")
	Dpanel:CreateBorder(false, true)
	G.UnitFrames.Player.Dpanel = Dpanel
	
	-- health  --
	G.UnitFrames.Player.Health:Size(1, 24)
	G.UnitFrames.Player.Health:ClearAllPoints()
	G.UnitFrames.Player.Health:Point("BOTTOMLEFT", Dpanel, "TOPLEFT", 0, 4)
	G.UnitFrames.Player.Health:Point("BOTTOMRIGHT", Dpanel, "TOPRIGHT", 0, 4)
	...
	if C["unitframes"].unicolor == true then
		...
		G.UnitFrames.Player.Health.bg:SetTexture(unpack(C["unitframes"].healthbg))
	...
	G.UnitFrames.Player.Health.value:SetPoint("RIGHT", G.UnitFrames.Player.Health, "RIGHT", -4, 1)
	G.UnitFrames.Player.Health.value:SetShadowOffset(0, 0)

	G.UnitFrames.Player.Health.PostUpdate = T.PostUpdateHealth

	-- power --
	G.UnitFrames.Player.Power:Size(C["unitframes"].playerwidth - 82, 5)
	G.UnitFrames.Player.Power:SetStatusBarTexture(C["media"].normTex)
	G.UnitFrames.Player.Power:ClearAllPoints()
	G.UnitFrames.Player.Power:Point("TOPRIGHT", Dpanel, "BOTTOMRIGHT", -2, -5)
	G.UnitFrames.Player.Power:SetFrameLevel(5)
	...
	G.UnitFrames.Player.Power.value = T.SetFontString(Dpanel, T.CreateFontString())
	G.UnitFrames.Player.Power.value:Point("LEFT", Dpanel, "LEFT", 2, -1)
	...
	-- powerlines --
	local l1 = CreateFrame("Frame", "l1", G.UnitFrames.Player)
	l1:Size(11, 2)
	l1:SetPoint("RIGHT", G.UnitFrames.Player.Power, "LEFT", -3, 0)
	l1:SetTemplate("Default")
	l1:SetFrameLevel(2)
	l1:SetFrameStrata("BACKGROUND")

	local l2 = CreateFrame("Frame", "l1", G.UnitFrames.Player)
	l2:Size(2, 5)
	...
	if C["unitframes"].charportrait then
		...
		pb:Size(43, 43)
		pb:SetPoint("BOTTOMRIGHT", G.UnitFrames.Player.Health, "BOTTOMLEFT", -3, -17)
	...
	if C["unitframes"].epbar then
		if T.level ~= MAX_PLAYER_LEVEL then
			...
			epBG:CreateBorder(true, true)
			epBG:SetFrameStrata("Background")
			epBG:CreateShadow("Default")
	...
	if T.level == MAX_PLAYER_LEVEL then
		...
		repBG:CreateShadow("Default")
	end

	-- combat feedback --
	if C["unitframes"].combatfeedback == true then G.UnitFrames.Player.CombatFeedbackText:SetFont(T.CreateFontString()) end
	...
	if T.myclass == "DRUID" then
		if C["unitframes"].druidmanabar then
			...
			G.UnitFrames.Player.DruidManaBackground:SetPoint("BOTTOM", G.UnitFrames.Player, "TOP", 0, -2)
			G.UnitFrames.Player.DruidManaBackground:Size(C["unitframes"].playerwidth, 2)
		...
		G.UnitFrames.Player.EclipseBar:SetPoint("BOTTOM", G.UnitFrames.Player, "TOP", 0, 5)
		G.UnitFrames.Player.EclipseBar:Size(C["unitframes"].playerwidth, 4)
		...
		if C["unitframes"].mushroombar == true then	
			...
			G.UnitFrames.Player.WildMushroom:SetPoint("BOTTOM", G.UnitFrames.Player, "TOP", 0, 13)
			G.UnitFrames.Player.WildMushroom:Size(C["unitframes"].playerwidth, 4)
			...
			for i = 1, 3 do
				if i == 1 then
					G.UnitFrames.Player.WildMushroom[i]:Size(T.Scale(C["unitframes"].playerwidth / 3) - 2, 4)
					G.UnitFrames.Player.WildMushroom[i]:SetPoint("LEFT", G.UnitFrames.Player.WildMushroom, "LEFT", 0, 0)
				else
					G.UnitFrames.Player.WildMushroom[i]:Size(T.Scale(C["unitframes"].playerwidth / 3), 4)
	...
	if T.myclass == "WARLOCK" then
		...
		G.UnitFrames.Player.WarlockSpecBars:Size(C["unitframes"].playerwidth, 4)
		...
		for i = 1, 4 do
			G.UnitFrames.Player.WarlockSpecBars[i]:Size(T.Scale(C["unitframes"].playerwidth / 4), 4)
	...
	if T.myclass == "PALADIN" then
		...
		G.UnitFrames.Player.HolyPower:Size(C["unitframes"].playerwidth, 4)
		...
		for i = 1, maxHolyPower do
			...
			if maxHolyPower == 3 then
				for i = 1, 3 do
					G.UnitFrames.Player.HolyPower[i]:Size(T.Scale(C["unitframes"].playerwidth / 3), 4)
				end
				G.UnitFrames.Player.HolyPower[i].width = G.UnitFrames.Player.HolyPower[i]:GetWidth()
			elseif maxHolyPower == 5 then
				if i == 5 then
					G.UnitFrames.Player.HolyPower[i]:Size(T.Scale(C["unitframes"].playerwidth / 5) - 2, 4)
				else
					G.UnitFrames.Player.HolyPower[i]:Size(T.Scale(C["unitframes"].playerwidth / 5), 4)
				end
				G.UnitFrames.Player.HolyPower[i].width = G.UnitFrames.Player.HolyPower[i]:GetWidth()
			end
	...
	if T.myclass == "MAGE" then
		...
		G.UnitFrames.Player.ArcaneChargeBar:Point("BOTTOMLEFT", G.UnitFrames.Player, "TOPLEFT", 0, 5)
		G.UnitFrames.Player.ArcaneChargeBar:Size(C["unitframes"].playerwidth, 4)
		...
		for i = 1, 6 do
			if i == 6 then
				G.UnitFrames.Player.ArcaneChargeBar[i]:Size(T.Scale(C["unitframes"].playerwidth / 6), 4)
			else
				G.UnitFrames.Player.ArcaneChargeBar[i]:Size(T.Scale(C["unitframes"].playerwidth / 6) - 1, 4)
			end
	...
	if T.myclass == "ROGUE" and C["misc"].sComboenable == false then
		...
		G.UnitFrames.Player.ComboPointsBar:SetPoint("BOTTOMLEFT", G.UnitFrames.Player, "TOPLEFT", 0, 5)
		G.UnitFrames.Player.ComboPointsBar:Size(C["unitframes"].playerwidth, 4)
		...
		for i = 1, 5 do
			if i == 5 then
				G.UnitFrames.Player.ComboPointsBar[i]:Size(T.Scale(C["unitframes"].playerwidth / 5) - 2, 4)
			else
				G.UnitFrames.Player.ComboPointsBar[i]:Size(T.Scale(C["unitframes"].playerwidth / 5), 4)
			end
	...
	if T.myclass == "DEATHKNIGHT" then
		...
		G.UnitFrames.Player.Runes:SetPoint("BOTTOMLEFT", G.UnitFrames.Player, "TOPLEFT", -1, 5)
		G.UnitFrames.Player.Runes:Size(C["unitframes"].playerwidth + 2, 4)
		...
		for i = 1, 6 do
			G.UnitFrames.Player.Runes[i]:Size(T.Scale(C["unitframes"].playerwidth / 6), 4)
			G.UnitFrames.Player.Runes[i].width = G.UnitFrames.Player.Runes[i]:GetWidth()
			if i == 1 then
				G.UnitFrames.Player.Runes[i]:Size(T.Scale(C["unitframes"].playerwidth / 6) - 4, 4)
				G.UnitFrames.Player.Runes[i].width = G.UnitFrames.Player.Runes[i]:GetWidth()
		...
		if C["unitframes"].showstatuebar then
			...
			G.UnitFrames.Player.Statue:Point("LEFT", G.UnitFrames.Player.Health, "RIGHT", 7, -4)
			G.UnitFrames.Player.Statue:SetOrientation'VERTICAL'
			G.UnitFrames.Player.Statue:Size(3, G.UnitFrames.Player.Health:GetHeight() + 7)
	...
	if T.myclass == "MONK" then
		...
		G.UnitFrames.Player.HarmonyBar:SetPoint("BOTTOMLEFT", G.UnitFrames.Player, "TOPLEFT", 0, 5)
		G.UnitFrames.Player.HarmonyBar:Size(C["unitframes"].playerwidth, 4)
		...
		for i = 1, maxChi do
			if maxChi == 4 then
				if i == 4 or i == 3 then
					G.UnitFrames.Player.HarmonyBar[i]:Size((C["unitframes"].playerwidth / 4) - 1, 4)
				else
					G.UnitFrames.Player.HarmonyBar[i]:Size(C["unitframes"].playerwidth / 4, 4)
				end
			elseif maxChi == 5 then
				if i == 5 then
					G.UnitFrames.Player.HarmonyBar[i]:Size(T.Scale(C["unitframes"].playerwidth / 5) - 1, 4)
				else
					G.UnitFrames.Player.HarmonyBar[i]:Size(T.Scale(C["unitframes"].playerwidth / 5), 4)
				end
			end

			if i == 1 then
				G.UnitFrames.Player.HarmonyBar[i]:SetPoint("BOTTOMLEFT", G.UnitFrames.Player, "TOPLEFT", 0, 5)
			else
	...
	if T.myclass == "SHAMAN" then
		for i = 1, 4 do
			...
			G.UnitFrames.Player.TotemBar[i]:Size(T.Scale((C["unitframes"].playerwidth - 2) / 4) - 5, 4)
	...
	if T.myclass == "PRIEST" then
		...
		G.UnitFrames.Player.ShadowOrbsBar:Size(C["unitframes"].playerwidth, 4)
		G.UnitFrames.Player.ShadowOrbsBar:CreateBackdrop("Default")
		G.UnitFrames.Player.ShadowOrbsBar.backdrop:CreateShadow("Default")

		for i = 1, 3 do
			G.UnitFrames.Player.ShadowOrbsBar[i]:Size(T.Scale((C["unitframes"].playerwidth - 1) / 3), 4)
		...
		if C["unitframes"].showstatuebar then
			...
			G.UnitFrames.Player.Statue:Point("LEFT", G.UnitFrames.Player.Health, "RIGHT", 7, -4)
			G.UnitFrames.Player.Statue:SetOrientation'VERTICAL'
			G.UnitFrames.Player.Statue:Size(3, G.UnitFrames.Player.Power:GetHeight() + 7)
	...
	if C["unitframes"].unitcastbar == true then
		...
		if C["unitframes"].cblatency == true then G.UnitFrames.Player.Castbar.SafeZone:SetVertexColor(0.69, 0.31, 0.31, 0.75) end
	end

	-- size
	G.UnitFrames.Player:Size(C["unitframes"].playerwidth, 41)
	
	-- target --
	-- panel --
	local DTpanel = CreateFrame("Frame", nil, G.UnitFrames.Target)
	DTpanel:SetTemplate("Default")
	DTpanel:Size(C["unitframes"].playerwidth + 2, 13)
	DTpanel:Point("BOTTOM", G.UnitFrames.Target, "BOTTOM", 0, 0)
	DTpanel:SetFrameLevel(2)
	DTpanel:SetFrameStrata("MEDIUM")
	DTpanel:CreateBorder(false, true)
	G.UnitFrames.Target.DTpanel = DTpanel
	
	G.UnitFrames.Target.Health:Size(1, 24)
	G.UnitFrames.Target.Health:ClearAllPoints()
	G.UnitFrames.Target.Health:Point("BOTTOMLEFT", DTpanel, "TOPLEFT", 0, 4)
	G.UnitFrames.Target.Health:Point("BOTTOMRIGHT", DTpanel, "TOPRIGHT", 0, 4)
	...
	if C["unitframes"].unicolor == true then
		...
		G.UnitFrames.Target.Health.bg:SetTexture(unpack(C["unitframes"].healthbg))
		if C["unitframes"].colorgradient then
			G.UnitFrames.Player.Health.colorSmooth = true
			G.UnitFrames.Player.Health.bg:SetTexture(.2, .2, .2)
		end	
	...
	G.UnitFrames.Target.Health.value:SetPoint("LEFT", G.UnitFrames.Target.Health, "LEFT", 4, 1)
	G.UnitFrames.Target.Health.value:SetShadowOffset(0, 0)

	G.UnitFrames.Target.Health.PostUpdate = T.PostUpdateHealth

	-- power --
	G.UnitFrames.Target.Power:Size(C["unitframes"].playerwidth - 82, 5)
	G.UnitFrames.Target.Power:SetStatusBarTexture(C["media"].normTex)
	G.UnitFrames.Target.Power:ClearAllPoints()
	G.UnitFrames.Target.Power:Point("TOPLEFT", DTpanel, "BOTTOMLEFT", 2, -5)
	G.UnitFrames.Target.Power:SetFrameLevel(5)
	...
	G.UnitFrames.Target.Name:SetPoint("LEFT", DTpanel, "LEFT", 4, 1)
	...
	-- powerlines --
	local l1 = CreateFrame("Frame", nil, G.UnitFrames.Target.Power)
	l1:Point("LEFT", G.UnitFrames.Target.Power, "RIGHT", 3, 0)
	l1:Size(11, 2)
	l1:SetTemplate("Default")
	l1:SetFrameLevel(2)
	l1:SetFrameStrata("BACKGROUND")
	...
	l2:Point("BOTTOMRIGHT", l1, "TOPRIGHT", 0, 0)
	l2:Size(2, 5)
	l2:SetTemplate("Default")
	l2:SetFrameLevel(2)
	l2:SetFrameStrata("BACKGROUND")
	...
	if C["unitframes"].charportrait then
		...
		local pbt = CreateFrame("Frame", G.UnitFrames.Target:GetName().."_PortraitBorder", G.UnitFrames.Target)
		pbt:CreateShadow("Default")
		pbt:Size(43, 43)
		pbt:SetPoint("BOTTOMLEFT", G.UnitFrames.Target.Health, "BOTTOMRIGHT", 3, -17)
		pbt:SetTemplate("Default")
		
		if C["unitframes"].portraitstyle == "MODEL" then
			local portrait = CreateFrame("PlayerModel", G.UnitFrames.Target:GetName().."_Portrait", pbt)
			portrait:SetFrameLevel(1)
			portrait:Point("TOPRIGHT", -2, 2)
			portrait:Point("BOTTOMLEFT", 2, 2)
			portrait.PostUpdate = T.PortraitUpdate --Worgen Fix (Hydra)
			G.UnitFrames.Target.Portrait = portrait
		else			
			local class = pbt:CreateTexture(G.UnitFrames.Target:GetName().."_ClassIcon", "ARTWORK")
			class:Point("TOPLEFT", -2, 2)
			class:Point("BOTTOMRIGHT", 2, 2)
	...
	if C["unitframes"].targetauras then
		if target then
			...
			buffs:SetWidth(C["unitframes"].playerwidth)
			buffs.size = 24.5
			buffs.num = 8
			buffs.spacing = 3
			...
			debuffs:SetWidth(C["unitframes"].playerwidth)
			debuffs.size = 24.5
			debuffs.num = 21
			debuffs.spacing = 3
	...
	-- size
	G.UnitFrames.Target:Size(C["unitframes"].playerwidth, 41)
	
	-- tot --
	-- health --
	G.UnitFrames.TargetTarget.Health:Size(C["unitframes"].playerwidth - 82, 15)
	...
	if C["unitframes"].totdebuffs == true then
		...
		G.UnitFrames.TargetTarget.Debuffs:Point("TOPLEFT", G.UnitFrames.TargetTarget, "BOTTOMLEFT", -1, -3)
	...
	-- size --
	G.UnitFrames.TargetTarget:Size(C["unitframes"].playerwidth - 82, 13)
	
	-- pet --
	-- health --
	G.UnitFrames.Pet.Health:Size(C["unitframes"].playerwidth - 82, 15)
	...
	-- size --
	G.UnitFrames.Pet:Size(C["unitframes"].playerwidth - 82, 13)
	
	-- focus --
	if C["unitframes"].lafo == false then
		-- health --
		G.UnitFrames.Focus.Health:Height(15)
		...
		G.UnitFrames.Focus.Name:Point("CENTER", G.UnitFrames.Focus.Health, "CENTER", 0, 0)
		G.UnitFrames.Focus.Name:SetShadowOffset(0, 0)
	else
		-- panel --
		local DFpanel = CreateFrame("Frame", nil, G.UnitFrames.Focus)
		DFpanel:SetTemplate("Default")
		DFpanel:Size(212, 13)
		DFpanel:Point("BOTTOM", G.UnitFrames.Focus, "BOTTOM", 0, 0)
		DFpanel:SetFrameLevel(2)
		DFpanel:SetFrameStrata("MEDIUM")
		DFpanel:CreateBorder(false, true)
		G.UnitFrames.Focus.DFpanel = DFpanel
		
		-- health  --
		G.UnitFrames.Focus.Health:Size(1, 24)
		G.UnitFrames.Focus.Health:ClearAllPoints()
		G.UnitFrames.Focus.Health:Point("BOTTOMLEFT", DFpanel, "TOPLEFT", 0, 4)
		G.UnitFrames.Focus.Health:Point("BOTTOMRIGHT", DFpanel, "TOPRIGHT", 0, 4)
		G.UnitFrames.Focus.Health:SetStatusBarTexture(C["media"].normTex)
		G.UnitFrames.Focus.Health:SetFrameLevel(5)
		...
		if C["unitframes"].unicolor == true then
			...
			G.UnitFrames.Focus.Health.colorDisconnected = false
			G.UnitFrames.Focus.Health.colorClass = false
			G.UnitFrames.Focus.Health:SetStatusBarColor(.125, .125, .125, 1)
			G.UnitFrames.Focus.Health.bg:SetTexture(unpack(C["unitframes"].healthbg))
		else
			G.UnitFrames.Focus.Health.colorDisconnected = true
			G.UnitFrames.Focus.Health.colorTapping = true
			G.UnitFrames.Focus.Health.colorClass = true
		...
		G.UnitFrames.Focus.Health.value:SetPoint("RIGHT", G.UnitFrames.Focus.Health, "RIGHT", -4, 1)
		G.UnitFrames.Focus.Health.value:SetShadowOffset(0, 0)

		G.UnitFrames.Focus.Health.PostUpdate = T.PostUpdateHealth

		-- power --
		G.UnitFrames.Focus.Power:Size(130, 5)
		G.UnitFrames.Focus.Power:SetStatusBarTexture(C["media"].normTex)
		G.UnitFrames.Focus.Power:ClearAllPoints()
		G.UnitFrames.Focus.Power:Point("TOPRIGHT", DFpanel, "BOTTOMRIGHT", -2, -5)
		G.UnitFrames.Focus.Power:SetFrameLevel(5)
		...
		G.UnitFrames.Focus.Power.value = T.SetFontString(DFpanel, T.CreateFontString())
		G.UnitFrames.Focus.Power.value:Point("LEFT", DFpanel, "LEFT", 2, -1)
		...
		-- powerlines --
		local l1 = CreateFrame("Frame", "l1", G.UnitFrames.Focus)
		l1:Size(11, 2)
		l1:SetPoint("RIGHT", G.UnitFrames.Focus.Power, "LEFT", -3, 0)
		l1:SetTemplate("Default")
		l1:SetFrameLevel(2)
		l1:SetFrameStrata("BACKGROUND")

		local l2 = CreateFrame("Frame", "l1", G.UnitFrames.Focus)
		l2:Size(2, 5)
		l2:SetPoint("BOTTOMLEFT", l1, "TOPLEFT", 0, 0)
		l2:SetTemplate("Default")
		l2:SetFrameLevel(2)
		...
		if C["unitframes"].charportrait then
			...
			pb:CreateShadow("Default")
			pb:Size(43, 43)
			pb:SetPoint("BOTTOMRIGHT", G.UnitFrames.Focus.Health, "BOTTOMLEFT", -3, -17)
			pb:SetTemplate("Default")
	...
	-- size
	if C["unitframes"].lafo == false then G.UnitFrames.Focus:Size(105, 10) else G.UnitFrames.Focus:Size(212, 41) end
	...
	-- size
	if C["unitframes"].lafo then G.UnitFrames.FocusTarget:Size(130, 10) else G.UnitFrames.FocusTarget:Size(105, 10) end
	
	-- boss --
	if C["unitframes"].showboss then
		for i = 1, MAX_BOSS_FRAMES do
			-- panel --
			local DBpanel = CreateFrame("Frame", nil, G.UnitFrames["Boss" .. i])
			DBpanel:SetTemplate("Default")
			DBpanel:Size(214, 13)
			DBpanel:Point("BOTTOM", G.UnitFrames["Boss" .. i], "BOTTOM", 0, 0)
			DBpanel:SetFrameLevel(5)
			DBpanel:SetFrameStrata("High")
			DBpanel:CreateBorder(false, true)
			G.UnitFrames["Boss" .. i].Dpanel = DBpanel
			
			-- power --
			G.UnitFrames["Boss" .. i].Power:Height(2)
			G.UnitFrames["Boss" .. i].Power:ClearAllPoints()
			G.UnitFrames["Boss" .. i].Power:Point("BOTTOMLEFT", DBpanel, "TOPLEFT", 2, 3)
			G.UnitFrames["Boss" .. i].Power:Point("BOTTOMRIGHT", DBpanel, "TOPRIGHT", -2, 3)
			...
			G.UnitFrames["Boss" .. i].Power:SetFrameLevel(5)
			...
			G.UnitFrames["Boss" .. i].Health:Size(216, 20)
			G.UnitFrames["Boss" .. i].Health:ClearAllPoints()
			G.UnitFrames["Boss" .. i].Health:Point("BOTTOMLEFT", G.UnitFrames["Boss" .. i].Power, "TOPLEFT", 0, 5)
			G.UnitFrames["Boss" .. i].Health:Point("BOTTOMRIGHT", G.UnitFrames["Boss" .. i].Power, "TOPRIGHT", 0, 5)
			...
			G.UnitFrames["Boss" .. i].Health:SetFrameStrata("High")
			...
			-- size
			G.UnitFrames["Boss" .. i]:Size(214, 41)
		end
	end
	...
	if C["unitframes"].arena then
		for i = 1, 5 do
			-- panel --
			local DApanel = CreateFrame("Frame", nil, G.UnitFrames["Arena" .. i])
			DApanel:SetTemplate("Default")
			DApanel:Size(214, 13)
			DApanel:Point("BOTTOM", G.UnitFrames["Arena" .. i], "BOTTOM", 0, 0)
			DApanel:SetFrameLevel(5)
			DApanel:SetFrameStrata("High")
			DApanel:CreateBorder(false, true)
			G.UnitFrames["Arena" .. i].DApanel = DApanel
			
			-- power --
			G.UnitFrames["Arena" .. i].Power:Height(2)
			G.UnitFrames["Arena" .. i].Power:ClearAllPoints()
			G.UnitFrames["Arena" .. i].Power:Point("BOTTOMLEFT", DApanel, "TOPLEFT", 2, 3)
			G.UnitFrames["Arena" .. i].Power:Point("BOTTOMRIGHT", DApanel, "TOPRIGHT", -2, 3)
			...
			G.UnitFrames["Arena" .. i].Power:SetFrameLevel(5)
			...
			G.UnitFrames["Arena" .. i].Health:Size(216, 20)
			G.UnitFrames["Arena" .. i].Health:ClearAllPoints()
			G.UnitFrames["Arena" .. i].Health:Point("BOTTOMLEFT", G.UnitFrames["Arena" .. i].Power, "TOPLEFT", 0, 5)
			G.UnitFrames["Arena" .. i].Health:Point("BOTTOMRIGHT", G.UnitFrames["Arena" .. i].Power, "TOPRIGHT", 0, 5)
			...
			G.UnitFrames["Arena" .. i].Health:SetFrameStrata("High")
			...
			if C["unitframes"].unitcastbar == true then
				...
				G.UnitFrames["Arena" .. i].Castbar:Point("TOP", DApanel, "BOTTOM", 10, -5)
			...
			-- size
			G.UnitFrames["Arena" .. i]:Size(214, 41)
		end
	end
	...
	FramePositions:SetScript("OnEvent", function(self, event, addon)
		...
		G.UnitFrames.TargetTarget:SetPoint("TOPLEFT", G.UnitFrames.Target, "BOTTOMLEFT", 1, -17)
		G.UnitFrames.Pet:SetPoint("TOPRIGHT", G.UnitFrames.Player, "BOTTOMRIGHT", -1, -17)
		if C["unitframes"].lafo then G.UnitFrames.Focus:SetPoint("TOPLEFT", UIParent, "TOPLEFT", 560, -370) else G.UnitFrames.Focus:SetPoint("TOPLEFT", G.UnitFrames.Player, "BOTTOMLEFT", -45, -5) end
		if C["unitframes"].lafo then G.UnitFrames.FocusTarget:SetPoint("TOPRIGHT", G.UnitFrames.Focus, "BOTTOMRIGHT", -2, -17) else G.UnitFrames.FocusTarget:SetPoint("TOPRIGHT", G.UnitFrames.Focus, "BOTTOMRIGHT", 0, -11) end
